# 👋 Yajat Gupta
### Computer Science Undergraduate | Data Science & Software Engineering  
📍 Jaipur, Rajasthan, India  

---

## 🔗 Connect With Me

| Platform | Link |
|--------|------|
| 💼 LinkedIn | https://www.linkedin.com/in/YajjatGupta |
| 💻 GitHub | https://github.com/YajjatGupta |
| 🧠 LeetCode | https://leetcode.com/YajjatGupta |
| 📧 Email | yajjatgupta@gmail.com |

---

## 🚀 About Me

Computer Science undergraduate specializing in **Data Science & Software Engineering** with strong foundations in:

- Java, Python, SQL
- Full-Stack Development
- Machine Learning Integration
- SDLC & Modular Code Design

Actively seeking **Software Engineering Internships**, **Data Science Roles**, and **Research Opportunities**.

---

## 🎓 Education

**Bennett University**, Greater Noida  
*B.E. Computer Science (Data Science & Analysis)*  
- CGPA: **8.5**
- **50% Merit Scholarship (JEE-based)**

**Maheshwari Public School**, Jaipur  
- CBSE XII (PCM): **89.8%**
- CBSE X: **90.1%**

---

## 💻 Technical Skills

### Programming Languages
- Python, Java, C++, JavaScript, SQL, HTML, CSS

### Frameworks & Tools
- React, Node.js, MySQL
- Git, Linux, Bash
- Tableau, Power BI

### Data Science & ML
- Pandas, NumPy, Scikit-Learn
- EDA, Model Integration

---

## 🚀 Projects

### 🎮 Project RPG
🔗 https://github.com/YajjatGupta/ProjectRPG  

- 2D RPG game with combat, AI, and level progression
- Modular architecture with improved scalability
- Reduced bug reports by **30%**

---

### 🧬 EDNA Project
🔗 https://github.com/YajjatGupta/Edna-Project  

- Web platform for species identification using deep learning
- Secure authentication and intuitive UI
- Improved researcher workflow by **40%**

---

## 💼 Experience

### Deloitte Australia – Data Analytics Job Simulation (Forage)

- Detected financial anomalies using structured & unstructured data
- Improved fraud detection accuracy by **25%**
- Built interactive Tableau dashboards

---

## 📜 Certifications

- Data Structures & Algorithms – Infosys Springboard
- Operating Systems and You – Coursera
- Databases & SQL for Data Science – Coursera
- Automated Software Testing with Python – Udemy

---

## 🤝 Contact

📧 Email: yajjatgupta@gmail.com  
💼 LinkedIn: https://www.linkedin.com/in/YajjatGupta

---
